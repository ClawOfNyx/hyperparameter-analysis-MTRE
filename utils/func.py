...
def read_data(model, dataset, split="val", prompt='oe', token_idx=0, return_data=False, num_samples=None):
    ...
    with open(f"./output/{model}/{dataset}_{split}_{prompt}.jsonl") as f:
        ...
        for line in tqdm(f):
            ...
            logits_key = 'logits' if 'logits' in data else 'logits_ref'
            raw_logits = data[logits_key]
            
            # Load numpy from files
            if isinstance(raw_logits, dict) and 'npy' in raw_logits:
                npy_path = raw_logits['npy']
                
                # Check if the path exists exactly as written
                if os.path.exists(npy_path):
                    raw_logits = np.load(npy_path)
                else:
                    fixed_path = os.path.join(".", npy_path.lstrip("./"))
                    if os.path.exists(fixed_path):
                        raw_logits = np.load(fixed_path)
                    else:
                        raise FileNotFoundError(f"Could not find the logits array file at: {npy_path}")
            
            if token_idx >= 0:
                x.append(raw_logits[token_idx])
            elif token_idx == -1:
                x.append(raw_logits[-1]) 
            else:
                x.append(raw_logits)
...
#                 continue
            # Fallback to 'logits_ref' if standard 'logits' key is missing
            logits_key = 'logits' if 'logits' in data else 'logits_ref'
            
            if token_idx >= 0:
                x.append(data[logits_key][token_idx])
            else:
                x.append(data[logits_key]) # Get the entire thing
            y.append(data['label'])
            if num_samples is not None and len(x) > num_samples:
                break
        print(line[:1000])
...
def read_data_non_linear(model, dataset, split="val", prompt='oe', token_idx=-1, return_data=False, num_samples=None, type_l='', keys_to_keep=None):
    ...
    with open(f"/content/scratch/output/{model}/{dataset}_{split}_{prompt}.jsonl") as f:
        dataset = []
        for line in tqdm(f):
            data = json.loads(line)
            if return_data:
            ...
            target_key = 'logits' + type_l
            
            # Load numpy from files
            raw_logits = data[target_key] if target_key in data else data.get('logits_ref' + type_l)
            if isinstance(raw_logits, dict) and 'npy' in raw_logits:
                npy_path = raw_logits['npy']
                if os.path.exists(npy_path):
                    raw_logits = np.load(npy_path)
            ...
            # Lead numpy from base logits
            base_logits = data['logits'] if 'logits' in data else data.get('logits_ref')
            if isinstance(base_logits, dict) and 'npy' in base_logits:
                npy_path = base_logits['npy']
                if os.path.exists(npy_path):
                    base_logits = np.load(npy_path)
                else:
                    fixed_path = os.path.join(".", npy_path.lstrip("./"))
                    if os.path.exists(fixed_path):
                        base_logits = np.load(fixed_path)
                    else:
                        raise FileNotFoundError(f"Could not find base logits array file at: {npy_path}")

            if token_idx == -1:
                x.append(raw_logits[-1])
                x_0.append(base_logits[-1])
            elif token_idx >= 0:
                x.append(raw_logits[token_idx])
            else:
                x.append(raw_logits)
                x_0.append(base_logits)

            label_val = data.get('label')
            y.append(0 if label_val is None else int(label_val))
            
            if num_samples is not None and len(x) > num_samples:
                break
            
    y = np.array(y) 
    if token_idx not in [-1, 0] and len(x) > 0 and isinstance(x[0], np.ndarray) and x[0].ndim > 1:
        pass 
    else:
        x = np.array(x)
        x_0 = np.array(x_0)
        
    print(f"Loaded {len(x)} samples. x is a {type(x)}, y shape: {y.shape}")
    return dataset, x, y, x_0
...
def analyze_data_distribution(x_train, y_train, x_val, y_val):
    ...
    if x_train.ndim == 3:
        zero_vector_mask_train = np.all(x_train == 0, axis=2)
        zero_vectors_per_sample_train = np.sum(zero_vector_mask_train, axis=1)
        average_zero_vectors_per_sample_train = np.mean(zero_vectors_per_sample_train)
        
        zero_vector_mask_val = np.all(x_val == 0, axis=2)
        zero_vectors_per_sample_val = np.sum(zero_vector_mask_val, axis=1)
        average_zero_vectors_per_sample_val = np.mean(zero_vectors_per_sample_val)
    else:
        zero_vectors_train = np.all(x_train == 0, axis=1)
        average_zero_vectors_per_sample_train = np.mean(zero_vectors_train.astype(float))
        
        zero_vectors_val = np.all(x_val == 0, axis=1)
        average_zero_vectors_per_sample_val = np.mean(zero_vectors_val.astype(float))
